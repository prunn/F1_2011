package com.prunn.rfdynhud.widgets.prunn.f1_2011.racecontrol;
...
public class RaceControlWidget extends Widget
{
    ...
    private final ImagePropertyWithTexture imgTitle = new ImagePropertyWithTexture( "imgName", "prunn/f1_2011/data_title.png" );
    private final ImagePropertyWithTexture imgTime = new ImagePropertyWithTexture( "imgTime", "prunn/f1_2011/race_control.png" );
    ...
    protected void initialize( LiveGameData gameData, boolean isEditorMode, DrawnStringFactory drawnStringFactory, TextureImage2D texture, int width, int height )
    {
        ...
        imgTitle.updateSize( Math.round(width * 0.25f), height / 2, isEditorMode );
        imgTime.updateSize( width, height / 2, isEditorMode );
        
        dsRC = drawnStringFactory.newDrawnString( "dsRC", width*5/100, height/4 - fh/2 + fontyoffset.getValue(), Alignment.LEFT, false, f1_2011Font.getFont(), isFontAntiAliased(), fontColor1.getColor(), null, "" );
        dsMessage = drawnStringFactory.newDrawnString( "dsMessage", width*3/100, height*3/4 - fh/2 + fontyoffset.getValue(), Alignment.LEFT, false, f1_2011Font.getFont(), isFontAntiAliased(), fontColor2.getColor(), null, "" );
    ...
    protected void drawWidget( Clock clock, boolean needsCompleteRedraw, LiveGameData gameData, boolean isEditorMode, TextureImage2D texture, int offsetX, int offsetY, int width, int height )
    {
        ...
        if(widgetpart == 1)
        {
            if ( needsCompleteRedraw || SCState.getValue() == YellowFlagState.PENDING)
                dsMessage.draw( offsetX, offsetY, Loc.sc_deployed, texture );
            else
                if ( needsCompleteRedraw || SCState.getValue() == YellowFlagState.LAST_LAP)
                    dsMessage.draw( offsetX, offsetY, Loc.sc_in, texture );
               
        }
        else
            if(widgetpart == 2 && DRS_enabled.getValue())
            {
                dsMessage.draw( offsetX, offsetY, Loc.drs_enabled, texture );
                //dsRC.draw( offsetX, offsetY, "Race Control", texture ); "DRS ENABLED"
            }    
            else
                if(widgetpart == 2 && !DRS_enabled.getValue())
                {
                    dsMessage.draw( offsetX, offsetY, Loc.drs_disabled, texture );
                }    
                else
                {
                    for(int i=0;i < numveh;i++)
        ...
        dsRC.draw( offsetX, offsetY, Loc.race_control, texture );
    ...
    {
        super( PrunnWidgetSetf1_2011.INSTANCE, PrunnWidgetSetf1_2011.WIDGET_PACKAGE_F1_2011_Race, 72.0f, 6.7f );
